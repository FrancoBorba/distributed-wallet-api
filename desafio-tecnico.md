...
Dinheiro não pode passar por float32 ou float64, nem durante parsing, cálculo, serialização ou persistência.
Idempotência deve ser persistente e sobreviver ao reinício de todos os processos.
As invariantes financeiras devem ser garantidas no banco, independentemente de locks locais e da deduplicação do SQS FIFO.
Eventos externos só podem ser publicados depois da confirmação da transação que os originou.
O ledger deve ser append-only: correções financeiras exigem novos lançamentos.
Carteiras independentes devem avançar em paralelo; locks globais são proibidos.
Atualizações de saldo devem impedir lost updates.
Unicidade, não negatividade e imutabilidade do ledger devem ser impostas pelo schema, pelas constraints e pelos mecanismos de proteção do banco.
6. Modelo de domínio
...
Envie a mesma aposta 50 vezes em paralelo e comprove um único débito.
Execute a disputa das duas apostas de 80.00 sobre saldo de 100.00.
Processe carteiras distintas simultaneamente.
Repita cenários relevantes com pelo menos três instâncias independentes.
Interrompa um consumidor depois do commit e antes da remoção da mensagem; valide a reentrega.
Execute dois publishers disputando a mesma outbox e valide a recuperação de publicação.
Entregue REFUND ou ROLLBACK antes da referência e comprove a resolução posterior ou a rejeição por expiração.
Reinicie a aplicação e verifique que idempotência, pendências e consistência financeira foram preservadas. Se houver aceite assíncrono, interrompa o processo após confirmar PENDING e antes de executar a operação; outra instância deve retomá-la.
